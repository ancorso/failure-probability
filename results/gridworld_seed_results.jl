...
ec = 1. # Exploration constant for MCTS
samples_per_leaf = 5 # Samples per leaf for computing failure prob MCTs
trials_per_size = 10
s0 = GridWorldState(6,6,1)

mdp, V, g_fail = create_sim(SeedGridWorld, w=10, h=10, T=30, s0=s0, Nwins = 15, p_val=0.15)
...
# Sample Solving
dpw_solver = DPWSolver(n_iterations=1000, depth=mdp.g.T+1, exploration_constant=1e3, k_state = .9, alpha_state = 0., check_repeat_state = false, estimate_value=myrollout, next_action = random_action, sample_ucb = true, k_action = 1., alpha_action = 0.3)
...
tree_sizes = Int.(floor.(10 .^ range(3, stop=5, length=5)))
...
for ts in tree_sizes
    ...
    for i in 1:trials_per_size
        ...
        dpw_solver1 = DPWSolver(n_iterations=ts, depth=mdp.g.T, exploration_constant=ec, k_state = .9, alpha_state = 0., check_repeat_state = false, estimate_value=myrollout, next_action = random_action, sample_ucb = false, k_action = 1., alpha_action = 0.3)
        dpw_planner1 = solve(dpw_solver1, mdp)
        action(dpw_planner1, UInt[])
        ...
        dpw_solver2 = DPWSolver(n_iterations=ts, depth=mdp.g.T, exploration_constant=ec, k_state = .9, alpha_state = 0., check_repeat_state = false, estimate_value=myrollout, next_action = random_action, sample_ucb = true, k_action = 1., alpha_action = 0.3)
        dpw_planner2 = solve(dpw_solver2, mdp)
        action(dpw_planner2, UInt[])
...
p1 = plot(minN:maxN, fill(V[to_index(s0, mdp.g.w)], Int(maxN - minN + 1)), title="Probability Estimates - Sample Std", xlabel="Rollout Budget", ylabel="Probability of Failure", label="Exact", xscale=:log10, yscale = :log10, legend=:bottomright)
...
savefig(p1, "gridworld_results_mean_sample_std.pdf")
