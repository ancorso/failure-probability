...
function DPWSolver(;depth::Int=10,
                    ...
                    sample_ucb::Bool = false,
                    double_reward::Bool = false,
                    skip_terminal_branches::Bool = false
                   )
    DPWSolver(depth, exploration_constant, n_iterations, max_time, k_action, alpha_action, k_state, alpha_state, keep_tree, enable_action_pw, check_repeat_state, check_repeat_action, tree_in_info, rng, estimate_value, init_Q, init_N, next_action, default_action, sample_ucb, double_reward, skip_terminal_branches)
end
...
mutable struct DPWTree{S,A}
    ...
    transitions2::Vector{Vector{Tuple{Int,Float64,Float64}}}
    a_labels::Vector{A}
    a_lookup::Dict{Tuple{Int,A}, Int}
    ρ::Vector{Float64}
    action_sequences::Vector{Vector{A}}
    skip::Set{Int}
    fail_sample::Vector{Float64}
...
function insert_action_node!(tree::DPWTree{S,A}, snode::Int, a::A, ρ::Float64, n0::Int, q0::Float64, maintain_a_lookup=true) where {S,A}
    push!(tree.n, n0)
    push!(tree.q, q0)
    push!(tree.q2, q0)
    push!(tree.fail_sample, NaN)
    push!(tree.a_labels, a)
    push!(tree.transitions, Vector{Tuple{Int,Float64}}[])
    push!(tree.transitions2, Vector{Tuple{Int,Float64,Float64}}[])
